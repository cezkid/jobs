...
# a grade the reader cannot check carries no information; the fact that earned it does (docs/resume/bullets.md)
GRADE_LIST = (
...
# one opening word leading this many bullets anywhere on the page reads as repetition (resume
# scorers flag it as "overused"). 4 is a convention, not a measured threshold
MAX_SAME_OPENING = 4
# filler wherever it appears. Common scorer lists add "the", "that", "which", "their" - ordinary
# English, left out - and match "lazy" inside "lazy loading", a technique: no word here is ever
# matched inside a compound
...
# British forms a US screener reads as misspellings, and automated scorers treat one spelling
# error as a hygiene failure for the whole page. Word -> US form, explicit forms
...
# chat can say when asked why. Mirrored in docs/resume/bullets.md #Why each rule - keep the two in step
WHY = {
...
def lint(model: dict, master: dict, inferences: list[dict] | None = None, posting: str = "") -> list[Finding]:
    ...
    for kind, where, text, entry_id in page_items(model):
        ...
        # "adds nothing" (1 false positive in 8 constructed clauses, docs/resume/bullets.md)
