...
MAX_BLOCK_WORDS = 57
# the user's call, 2026-09-24, after VMock's tip of a 4-6 line summary: up to four rows at the
# summary's width. It was two, set by the tailoring prompt; the word cap above was never the
# limit - 57 words already run to about five rows in Caladea (~12 words a row at 90% width)
MAX_SUMMARY_LINES = 4
MAX_BYTES = 1_000_000
MAX_PAGES = 2
...
# scanners, as a space. Measured 2026-09-24: VMock split the headings, set at +0.08em, into
# "EXP E R I ENC E" and "SKI L L S", and read every body word, at +0.015em, whole. PyMuPDF,
# pdfminer and pypdf all read both whole, so no extractor here can stand in for VMock's; the
# gap itself is what the gate measures. 0.04em sits between the two measured points
MAX_LETTER_GAP_EM = 0.04
# a text span this dark or darker is black to a colour check: the page's ink (#1A1A1A) passed
# VMock's "all text should be black" check on 2026-09-24, navy #1F3A5F failed it. Links may
# carry colour - VMock allows a blue email or LinkedIn - so spans on a link are not held to it
